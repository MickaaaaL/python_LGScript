...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.charger is not None:
        try:
            ...
            logger.warning("Chargement initial échoué : %s", exc)
            print(f"❌ {exc}")
    ...
    while True:
        ...
        if choix == "1":
            ...
            try:
                ...
                logger.warning("Ajout refusé : %s", exc)
                print(f"❌ {exc}")
                continue
            ...
            fmt = input(f"Format [{args.format}] : ").strip().lower() or args.format
            try:
                if fmt in ("c", "csv"):
                ...
                logger.warning("Erreur d'écriture : %s", exc)
            ...
            try:
                ...
                logger.warning("Chargement échoué : %s", exc)
                print(f"❌ {exc}")
                continue
