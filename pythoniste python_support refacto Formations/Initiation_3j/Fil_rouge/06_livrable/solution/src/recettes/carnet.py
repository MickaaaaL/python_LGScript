...
def ajouter_recette(carnet: list[dict], recette: dict) -> None:
    ...
    logger.info("Recette ajoutée : %s", recette["nom"])
...
def sauvegarder(carnet: list[dict], chemin: Path) -> None:
    ...
    logger.info("Sauvegarde : %d recettes dans %s", len(carnet), chemin)
...
def charger(chemin: Path) -> list[dict]:
    ...
    logger.info("Chargement : %d recettes depuis %s", len(donnees), chemin)
    return donnees
