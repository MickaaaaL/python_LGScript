...
def csv_with_errors(tmp_path: Path) -> Path:
    """Cree un CSV avec des lignes invalides."""
...
def inbox_dir(tmp_path: Path) -> Path:
    """Cree un dossier inbox avec plusieurs CSV."""
...
class TestParseCsv:
    ...
    def test_csv_with_errors_skips_invalid(self, csv_with_errors: Path) -> None:
        ...
        assert len(readings) == 2  # la ligne invalide est ignoree
